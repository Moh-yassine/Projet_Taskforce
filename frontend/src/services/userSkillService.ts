...
class UserSkillService {
  private readonly API_BASE_URL = '/api/user-skills';
  ...
  async updateUserSkill(id: number, data: UpdateUserSkillData): Promise<UserSkill> {
    ...
    if (!response.ok) {
      ...
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erreur lors de la mise à jour de la compétence utilisateur');
    }
  ...
  async deleteUserSkill(id: number): Promise<{ message: string }> {
    ...
    if (!response.ok) {
      ...
      const errorData = await response.json();
      throw new Error(errorData.error || 'Erreur lors de la suppression de la compétence utilisateur');
    }
